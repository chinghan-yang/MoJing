"""
YOLO11 Pose 視訊背景＋影片素材水波＋貓咪淡入（提早淡入 & 徑向偏移版）
Python 3.11

需求調整：
1. [新增] --cat_appear_phase: 控制水波播放到幾成時生成貓咪 (預設 0.7 = 70%)。
2. [新增] --cat_offset_radial: 控制貓咪沿半徑往圓心內縮的距離 (正值往內，負值往外)。

執行示例：
  python yolo11_pose_overlap_offset.py \
...
class ActiveRipple:
    ...
    cat_spawned: bool = False  # [新增] 標記是否已觸發貓咪生成
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLO11 Pose｜提早淡入 & 徑向偏移版")
    ...
    # [新增參數]
    p.add_argument("--cat_appear_phase", type=float, default=0.8, help="水波播放進度達到多少時生成貓咪 (0.0~1.0)")
    p.add_argument("--cat_offset_radial", type=float, default=30.0,
                   help="貓咪位置往圓心偏移的距離 (像素, 正值往內, 負值往外)")
...
def main():
    ...
    win_name = "Video BG + Overlap & Offset"
    ...
    while True:
            ...
            if rp.frame_idx < ripple_total_frames:
                ...
                if progress >= args.cat_appear_phase and not rp.cat_spawned:
                    ...
                    if target and cats_loaded:
                        ...
                        rp.cat_spawned = True  # 標記已生成，避免重複

                rp.frame_idx += 1
            else:
                finished.append(rp)

        # 移除已播完的水波 (因為貓咪已在中途生成，這裡只需清理物件)
        for rp in finished:
            running_ripples.remove(rp)
            # 若水波結束時還沒生成貓 (例如 progress 設為 1.0)，可在此補救，
            # 但一般 logic 在上面都已涵蓋。
        ...
        for tr in tracks.values():
            ...
            # [新增] 2. 應用徑向偏移 (Radial Offset)
            # 計算從切點往圓心的向量 (Center - Target)
            dir_x = center[0] - target_x
            dir_y = center[1] - target_y
            norm = (dir_x ** 2 + dir_y ** 2) ** 0.5
            if norm > 0.001:
                ux, uy = dir_x / norm, dir_y / norm
                # 加上偏移量 (args.cat_offset_radial > 0 往圓心, < 0 往圓外)
