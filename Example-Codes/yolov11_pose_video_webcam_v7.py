        ...
        while True:
            ...
            if result.keypoints.has_visible:
                for i, person_kpts in enumerate(keypoints_xy):
                    ...
                    # 【重要】 如果沒有任何頭部節點，跳過此人 (不紀錄、不傳輸)
                    if not has_any_head:
                        continue

                    target_point = None

                    # --- 【步驟 2】 決定角度計算的基準點 (依據優先順序) ---
                    # 規則 6: 若有任何頭部節點 (已確認) 且有雙肩，優先使用雙肩中心
                    ...
                    # 再次確認 (理論上上面 has_any_head 已經擋掉空值，但保險起見)
                    if target_point is None:
                        continue

                    # --- 【步驟 3】 計算角度 (傳入 is_mirror_mode) ---
