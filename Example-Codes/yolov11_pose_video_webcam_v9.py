    ...
    try:
        while True:
            ...
            if is_mirror_mode:
                ...
                # 注意：如果使用鏡像翻轉，遮罩 (well_mask) 是否也需要翻轉取決於遮罩的製作方式
                # 通常遮罩是對應「畫面看到」的樣子。如果遮罩是基於未翻轉畫面製作的，這裡可能需要同步翻轉遮罩
                # 假設遮罩是已經對應好「最終顯示畫面」的，則不需額外處理
                    ...
                    if well_mask is not None:
                        for k_idx, (kx, ky) in enumerate(person_kpts):
                            ...
                            if 0 <= iy < well_mask.shape[0] and 0 <= ix < well_mask.shape[1]:
                                # 如果遮罩值為 1 (代表遮蔽)，則將信心值設為 -1
                                if well_mask[iy, ix] == 1:
                                    masked_confs[k_idx] = -1.0

                    # 後續邏輯改用 masked_confs 來判斷
