"""
YOLO11 Pose 視訊背景＋條件觸發水波＆貓咪淡入（修正版）
Python 3.11

修正內容（針對「有水波但無貓淡入」）：
- 以 **cv2.IMREAD_UNCHANGED** 載入 PNG，保留 Alpha；
- 覆蓋時正確處理 **每像素 Alpha × 淡入係數**（非單一 addWeighted）；
- 新增路徑健壯性：若 --cats 未指定或路徑讀不到，會嘗試自動尋找 `./cats/*.png|jpg` 或 `./cat*.png|jpg`；並在畫面左上角顯示警告；
- 顯示目前載入到的貓圖數量，方便確認；
- 其他小修：避免座標越界、完成淡入後維持顯示。

需求重點：
- 背景=mp4 循環播放；
- 人臉 5 點（鼻、左眼、右眼、左耳、右耳）同時存在並連續 3 秒→以鼻子座標觸發水波＋隨機貓圖 3 秒淡入；
- 保留圓形視窗與（可選）骨架疊圖。

執行範例：
  python yolo11_pose_video_ripple_cats_v1.py \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="YOLO11 Pose｜影片背景＋條件觸發水波＆貓咪淡入（修正版）")
    ...
    p.add_argument("--cats", type=str, default="", help="以逗號分隔的貓圖路徑；若留空會自動搜尋 ./cats 與 ./cat*.png|jpg")
...
def overlay_cat(canvas: np.ndarray, cat: CatOverlay, now: float):
    """將貓圖以 3 秒淡入方式疊加：
       - 若 PNG 具 Alpha：使用 (alpha_channel * t) 當作每像素透明度；
       - 否則：使用全域 t 當權重。
    """
    t = cat.alpha(now)
    if t <= 0.0:
...
def main():
    ...
    while True:
        ...
        if candidates:
            sel = choose_locked_person(locked_center, centers)
        ...
        if hold_start is not None and (now - hold_start) >= args.face_hold_sec and (now - last_trigger) >= args.trigger_cooldown:
            ...
            nose_x = int(np.clip(nose_x, 0, w - 1))
            nose_y = int(np.clip(nose_y, 0, h - 1))
            ...
            if cats_loaded > 0:
                ...
                target_h = max(1, int(cat_img.shape[0] * scale))
                x = max(0, min((w - target_w) // 2, w - target_w))
                y = max(0, min(h - target_h - int(h * 0.05), h - target_h))
        ...
        cv2.putText(masked, f"FPS: {fps:.1f}", (16, 36), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(masked, f"Cats loaded: {cats_loaded}", (16, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        if cats_loaded == 0:
            cv2.putText(masked, "No cat images found. Use --cats or put files into ./cats/", (16, 96), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2, cv2.LINE_AA)
